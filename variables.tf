variable "cloud_settings" {
  type = object({
    ...
    interface_used_as_admin         = string
    cluster_name                    = string
  ...
  description = <<-EOL
    ...
    device_name                     : Name (and hostname) of the MikroTik device (string)
    device_ip                       : IP of device on the admin network (Default: "192.168.88.1")
    interface_used_as_admin         : Limit administration interfaces to a port, e.g. same as cloud_name (string)
    ...
    certificate_domain              : Domain name of the device. Will be used with device_name when creating certificates (string)
...
variable "routing" {
  ...
  type = object({
    ...
    dhcp_dns_servers         = optional(set(string), ["1.1.1.1", "1.0.0.1"])
    node_ip_pool             = optional(list(string), ["10.25.0.10-10.25.0.99"])
  ...
  description = <<-EOL
    ...
    dhcp_dns_servers         : List of DNS servers published to clients configured by DHCP (Default: ["1.1.1.1", "1.0.0.1"])
